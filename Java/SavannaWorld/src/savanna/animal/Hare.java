...
public class Hare extends Animal implements Liveable {


    private final int STARVE_TIME = 2;
    private final double REPRODUCTION_RATE = 0.25;
    ...
    public void attemptToReproduce() {
        if (this.reproductionRate >= 1) {
            if (!grid.getEmptyAdjacentLocations(this.getLocation()).isEmpty()) {
                grid.put(grid.getEmptyAdjacentLocations(this.getLocation()).get(0), new Hare());
                this.reproductionRate = 0;
            } else {
                return;
            }
